...
/**
 * \defgroup Private_Heap Internal structures used by the heap allocator (allocator_malloc)
 * @{
 */
 ...
 * @struct __node__ 
 *
 * @brief Internal node structure used used by the intrusive linked list.
 */
struct __node__
{
	/** previous node */
 ...
 * @struct reg_obj
 *
 * @brief Internal registration object used to manage the list of registered objects in the allocator. 
 */
struct reg_obj
{
	/** node */
...
struct shared_data
{
	...
	/**
         * Size of contiguous block of memory.
         */
	size_t size;

	/**
         * Used to represent null pointer.
         */
	char __null__;
...
void *base_address;
      	...
      	if ( (base_address = malloc(size + sizeof(struct shared_data))) == NULL)
