...
int main (int argc, char *argv[])
{
	...
	/* Create an allocator */
	cn_allocator_t *alloc_ptr = cn_allocator_shm_create( 1024 * 1024, TEST_ALLOCATOR_SHM_KEY );
	...
	cn_shared_in_list_t *sl_ptr = cn_shared_in_list_create("My Shared Intrusive List", alloc_ptr);
	...
	sl_ptr->add_tail( sl_ptr, (cn_shared_in_list_node_t *) m_ptr ); 
	...
	memmove(dst_ptr,sd_ptr, alloc_ptr->get_size(alloc_ptr) );
        ...
        alloc_ptr =  cn_allocator_malloc_attach(dst_ptr);
	...
	/* attached to memory */	
	sl_ptr = cn_shared_in_list_attach("My Shared Intrusive List", alloc_ptr);
